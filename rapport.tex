...
\begin{itemize}
	...
	\item $\deg{l\LM(g_i)r} = \deg{l'\LM(g_j)r'}$ et $i = j$ et $\deg{r} = \deg{r'}$ et $l \leq l'$.
\end{itemize}
...
\begin{proof}
	...
	$$\left(- (x^n, 2, 1)
		...
		, ta^nb - ta^{n+1} \right)$$
	est dans $M$ donc doit être réductible par un élément $(\sigma_n, f_n)$ de $G_f$, il existe donc $l, r \in \M$ tels que~:
	\begin{align*}
		l\LM(f_n)r = ta^nb \\
	...
	\begin{itemize}
		...
		% Ce terme peut seulement apparaître en utilisant un 3 avec quelqu'un du même degré (ce n'est pas forcément le monôme dominant).
		
		\item Cas $\LM(f_n) = ta^k$.
		
		% On peut seulement le réécrire en des trucs lexicographiquement plus grands !
	\end{itemize}
...
\begin{proof}
	...
	% TODO : argumenter que s est réductible
