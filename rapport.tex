...
\begin{itemize}
	...
	\item $\deg{l\LM(g_i)r} = \deg{l'\LM(g_j)r'}$ et $i \leq 2$ et $j = 3$.
	\item $\deg{l\LM(g_i)r} = \deg{l'\LM(g_j)r'}$ et ($i, j \leq 2$ ou $i = j = 3$) et $\deg{r} < \deg{r'}$.
	\item $\deg{l\LM(g_i)r} = \deg{l'\LM(g_j)r'}$ et ($i, j \leq 2$ ou $i = j = 3$) et $\deg{r} = \deg{r'}$ et $l\LM(g_i)r \geq l'\LM(g_j)r'$.
	% Penser à xxt xta taa tab xtb pour voir que le signe doit être dans l'autre sens.
	\item $\deg{l\LM(g_i)r} = \deg{l'\LM(g_j)r'}$ et ($i, j \leq 2$ ou $i = j = 3$) et $\deg{r} = \deg{r'}$ et $l\LM(g_i)r = l'\LM(g_j)r'$ et $i \leq j$.
\end{itemize}
...
\end{lem}
\begin{proof}
	Considérons une syzygie homogène $s$, nous voulons montrer qu'elle est réductible par $\sigma$ ou par l'une des syzygies principales. 
	
	Supposons que $s$ n'est réductible par aucune syzygie principale.
	
	Soit $(l, i, r)$ le monôme dominant de $s$.
	
	% Il faut retransformer la preuve pour parler des cycles de réécriture à la place des syzygies homogènes :
	% Toute syzygie s'écrit comme combinaison linéaire de cycles de réécriture : on prend  le monome dominant de la syzygie, et on réduit la syzygie par un sous-cycle de la syzygie de telle manière à enlever l'arc du monôme dominant. On répète jusqu'à obtenir 0.
	
	% Si le monôme dominant est un 3, alors soit il y a un t à gauche de la réécriture auquel cas on peut le réécrire par sigma, soit le a réécrit est dans le "garbage" (il ne peut pas être réécrit par aucun t), le b obtenu doit être retransformé en a par la même règle, et alors on remarque que les hypothèses sur l'ordre font que le contexte est le même dans les deux cas : les deux termes sont identiques, contradiction.
	
	% Sinon, s'il n'y a aucun 3, alors, comme  toutes les syzygies sont principales car elles agissent toutes sur des t différents (et donc commutent).
	% En particulier, on peut considérer le premier t : les première réécritures se font dessus. Comme on n'utilise pas la règle 3, il est impossible de créér une syzygie non-nulle.
	
	Finalement n'importe quelle syzygie $s$ est réductible, car la syzygie constituée des termes du plus haut degré est réductible (car homogène), et la même réduction fonctionne sur $s$.
\end{proof}
